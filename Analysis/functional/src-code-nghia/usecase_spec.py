...
parser.add_argument('--tex_folder', type=str, default='/media/vutrungnghia/New Volume/InformationSystemAnalysisAndDesign/system-analysis-design/Analysis/functional/usecase-spec/gamer')
...
s = open(os.path.join(args.tex_folder, 'template.txt')).read()
s = s.split('%=%=%=%')
table_tex = s[0] + result + s[1]
path = os.path.join(args.tex_folder, f'{args.table_name}.tex')

if os.path.isfile(path):
    raise RuntimeError(f'{path} existed')
